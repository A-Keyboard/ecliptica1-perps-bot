...
Changes in v0.6.2
──────────────────
• `/ask` off‑loads the blocking `rei_call()` via `asyncio.get_running_loop().run_in_executor`, fixing the AttributeError from `.run_async()`.
• Updated REI endpoint to `/v1/chat/completions` with 60 s timeout.
• Removed rate‑limiter dependency; runs on base python-telegram-bot install.
...
async def ask_next(update_or_q, ctx):
    ...
    key, q = QUESTS[i]
    await update_or_q.message.reply_text(f"[{i+1}/{len(QUESTS)}] {q}")
    return SETUP

async def collect(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data["ans"][QUESTS[ctx.user_data["i"]][0]] = update.message.text.strip()
    ctx.user_data["i"] += 1
    return await ask_next(update, ctx)
